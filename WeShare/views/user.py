...
def index(username):
    ...
    pagination = Share.query.with_parent(user).order_by(Share.timestamp.desc()).paginate(page, per_page)
    shares = pagination.items
    return render_template('user/index.html', user=user, pagination=pagination, shares=shares)
