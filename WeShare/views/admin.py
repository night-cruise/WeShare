...
def index():
    ...
    return render_template(
        ...
        share_count=share_count, reported_shares_count=reported_shares_count,
...
def manage_user():
    ...
    per_page = current_app.config['WESHARE_MANAGE_USER_PER_PAGE']
...
@admin_bp.route('/manage/share', defaults={'order': 'by_flag'})
@admin_bp.route('/manage/photo/<order>')
@login_required
@permission_required('MODERATE')
def manage_share(order):
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['WESHARE_MANAGE_SHARE_PER_PAGE']
    order_rule = 'flag'
    if order == 'by_time':
        pagination = Share.query.order_by(Share.timestamp.desc()).paginate(page, per_page)
        order_rule = 'time'
    else:
        pagination = Share.query.order_by(Share.flag.desc()).paginate(page, per_page)
    shares = pagination.items
    return render_template('admin/manage_share.html', pagination=pagination, shares=shares, order_rule=order_rule)
...
def manage_tag():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['WESHARE_MANAGE_TAG_PER_PAGE']
...
def manage_comment(order):
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['WESHARE_MANAGE_COMMENT_PER_PAGE']
    order_rule = 'flag'
    if order == 'by_time':
