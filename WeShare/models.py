...
class Role(db.Model):
    ...
    def init_role():
        roles_permissions_map = {
            ...
            'User': ['FOLLOW', 'COLLECT', 'COMMENT', 'SHARE'],
            'Moderator': ['FOLLOW', 'COLLECT', 'COMMENT', 'SHARE', 'MODERATE'],
            'Administrator': ['FOLLOW', 'COLLECT', 'COMMENT', 'SHARE', 'MODERATE', 'ADMINISTER']
        }
...
class Collect(db.Model):
    collector_id = db.Column(db.Integer, db.ForeignKey('user.id'),
                             primary_key=True)
    collected_id = db.Column(db.Integer, db.ForeignKey('share.id'),
                             primary_key=True)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
...
class User(db.Model, UserMixin):
    ...
    shares = db.relationship('Share', back_populates='author', cascade='all')
    ...
    def set_role(self):
        if self.role is None:
            if self.email == current_app.config['WESHARE_ADMIN_EMAIL']:
    ...
    def collect(self, share):
        if not self.is_collecting(share):
            collect = Collect(collector=self, collected=share)
            db.session.add(collect)
            db.session.commit()

    def uncollect(self, share):
        collect = Collect.query.with_parent(self).filter_by(collected_id=share.id).first()
        if collect:
            db.session.delete(collect)
            db.session.commit()

    def is_collecting(self, share):
        return Collect.query.with_parent(self).filter_by(collected_id=share.id).first() is not None
...
class Share(db.Model):
    ...
    author = db.relationship('User', back_populates='shares')
    comments = db.relationship('Comment', back_populates='share', cascade='all')
