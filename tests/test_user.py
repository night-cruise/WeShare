...
class UserTestCase(BaseTestCase):
    ...
    def test_change_email(self):
        user = User.query.get(2)
        self.assertEqual(user.email, 'normal@weshare.com')
        token = generate_token(user=user, operation=Operations.CHANGE_EMAIL, new_email='new@weshare.com')
    ...
    def test_notification_setting(self):
        ...
        self.client.post(url_for('main.new_comment', share_id=2), data=dict(body='test comment from admin user.'))
        self.client.post(url_for('main.collect', share_id=2))
