...
from WeShare.models import User, Role, Tag
...
class AdminTestCase(BaseTestCase):
    ...
    def test_index_page(self):
        ...
        self.assertIn('WeShare Dashboard', data)
    ...
    def test_bad_permission(self):
        ...
        self.assertNotIn('WeShare Dashboard', data)
    ...
    def test_edit_profile_admin(self):
        ...
        self.assertEqual(user.email, 'new@weshare.com')
    ...
    def test_delete_tag(self):
        tag = Tag(name='test tag 2')
        db.session.add(tag)
        db.session.commit()
        self.assertIsNotNone(Tag.query.get(2))

        response = self.client.post(url_for('admin.delete_tag', tag_id=2), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Tag deleted.', data)
        self.assertEqual(Tag.query.get(2), None)
    ...
    def test_manage_share_page(self):
        response = self.client.get(url_for('admin.manage_share'))
        data = response.get_data(as_text=True)
        self.assertIn('Manage Shares', data)
        self.assertIn('Order by flag <span class="oi oi-elevator"></span>', data)

        response = self.client.get(url_for('admin.manage_share', order='by_time'))
        data = response.get_data(as_text=True)
        self.assertIn('Manage Share', data)
