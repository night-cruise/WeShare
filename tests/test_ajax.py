...
class AjaxTestCase(BaseTestCase):
    ...
    def test_collectors_count(self):
        response = self.client.get(url_for('ajax.collectors_count', share_id=1))
        data = response.get_json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['count'], 0)
    ...
    def test_collect(self):
        response = self.client.post(url_for('ajax.collect', share_id=1))
        data = response.get_json()
        self.assertEqual(response.status_code, 403)
        self.assertEqual(data['message'], 'Login required.')
        ...
        data = response.get_json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['message'], 'Share collected.')
    ...
    def test_uncollect(self):
        response = self.client.post(url_for('ajax.uncollect', share_id=1))
        data = response.get_json()
        self.assertEqual(response.status_code, 403)
        self.assertEqual(data['message'], 'Login required.')
